    ...
    public:
        MemoryManager(const std::string &config_path) : cfg_(config_path){
            ...
            for(std::map<std::string, int>::iterator it = cfg_.buffer_config.begin();
                ...
                ptr = nmap(0, SIZE, PROT_WRITE, MAP_SHARED,shm_file_desc, 0);
                if(ptr == MAP_FAILED)
                ...
                for(std::map<std::string, YAML::Node>::iterator it = cfg_.controller_config.begin(); it != cfg_.controller_config.end(); ++it){
                    ...
                    if(it->second["type"].as<std::string>() == "onnx"){
                        ...
                        std::shared_ptr<OnnxController> onnx_controller =std::maked_shared<OnnxController>(observations, actions, model_path);
        ...
        ~MemoryManager(){
           ...
           int create_buffer(const char *name, const int &rw, T *&ptr){
            ...
            int shared_memory_fd = shm_open(name, O_RDWR, S_IRWXU, S_IRWXO);
           ...
           void *mapped_memory = nmap(0, buffer_size, protection_level, MAP_SHARED, shared_memory_fd,0);
